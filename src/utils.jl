using Mooncake, DifferentiationInterface, Optim

export build_grid, DiscreteMeasure, sparsity, build_ηV, complex_grad, λ₀

function build_grid(domain::AbstractVector{T}, n_grid::Int) where {T<:Real}
    a, b = domain
    step = (b - a) / n_grid
    range(a, b - step; length=n_grid)
end

function build_grid(domain::AbstractVector{<:AbstractVector{<:Real}}, n_grid::Int)
    d = length(domain)
    ...
    elseif d == 2
        rx = range(domain[1][1], step=(domain[1][2] - domain[1][1]) / n_grid, length=n_grid)
        ry = range(domain[2][1], step=(domain[2][2] - domain[2][1]) / n_grid, length=n_grid)
