...
noise_levels = 0.01 .+ (0.501 - 0.001) * (range(0, stop=1, length=num_ns_lvls) .^ 2)
λs = 0.005noise_levels

markersizes = 0.1noise_levels

red_cmap = cgrad([RGB(0, 0, 0), RGB(1, 0, 0)])
colors = get(red_cmap, 1:num_ns_lvls, :extrema)

for (i, noise_level) in enumerate(noise_levels)
    Random.seed!(1)
    prob.y = add_noise(y0, noise_level)
    prob.λ = λs[i]
    solve!(prob, :BSFW, options=Dict(:maxits => 5, :positivity => true))
...
savefig(plt_cert, "figures/PreCertificates/Noise_robustness.svg")